...
#define LIMIT_AMPLITUDE 1  /*  if any of the phase space variables (except the
			       sixth N.C.) exceeds this limit it is marked as
			       lost */


typedef int*(*MYPASS)(mxArray*, int*, double*, int, int);
typedef struct elem*(*MYTRACK)
  (mxArray*,struct elem*, double*, int, struct parameters*);
...
{
  /* recusively search the list to check if the library containing method_name
     is already loaded. If it is - retutn the pointer to the list element. If
     not - return NULL */
  if (head)
...
static struct LibraryListElement* pass_method(mxArray *mxPassMethod, int nelem)
{
  ...
  struct LibraryListElement *LibraryListPtr =
    SearchLibraryList(LibraryList, fn_name);
  ...
  if (!LibraryListPtr) {
    ...
    LibraryListPtr =
      (struct LibraryListElement*)mxMalloc(sizeof(struct LibraryListElement));
    ...
    case 3: /* mex-file not on the search path */
      ...
      LibraryListPtr->PassHandle =
	(MYPASS)GETPASSFCN(LibraryListPtr->LibraryHandle);
      LibraryListPtr->TrackHandle =
	(MYTRACK)GETTRACKFCN(LibraryListPtr->LibraryHandle);
      if ((LibraryListPtr->PassHandle == NULL)
	  && (LibraryListPtr->TrackHandle == NULL)) {
	FREELIBFCN(LibraryListPtr->LibraryHandle);
	mexErrMsgIdAndTxt
	  ("Atpass:UnknownLibrary",
	   "Element # %d: Library file: %s, No passFunction or trackFunction"
	   " available", nelem, LibraryFileName);
      }
      mxFree((void *)LibraryFileName);
      break;
    default:
      mexErrMsgIdAndTxt
	("Atpass:UnknownPassMethod",
...
static void
checkiflost(mxDouble *DblBuffer, int np, mxDouble num_elem, mxDouble num_turn,
	    mxDouble *xnturn, mxDouble *xnelem, mxDouble *xcoord,
	    mxDouble *xlostcoord, mxLogical *xlost, mxDouble *histbuf,
	    int ihist, int lhist)
{
  int n, c;
  for (c=0; c<np; c++) {/* Loop over particles */
    if (!xlost[c]) {  /* No change if already marked */
      ...
      for (n=0; n<6; n++) {
	/* I remove the check on the sixth coordinate N.C. */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  mxArray *mxBuffer;
  const char *lossinfo[] =
    {"lost", "turn", "element", "coordinates_at_loss", "coordinates"};
  ...
  if (nlhs >= 2) {
    ...
    if (lhist < 0) {
      mexErrMsgIdAndTxt("Atpass:WrongParameter",
			"History length must be non-negative");
    }
    else if (lhist > 0) {
  ...
  if (new_lattice) {
    ...
    for (nelem=0; nelem<num_elements; nelem++) {
      /* free memory from previously used lattice */
      mxFree(field_numbers_ptr[nelem]);
      mxFree(elemdata_list[nelem]);
    }        
    num_elements = numel;
        
    /* Pointer to integer maps of Element data fields used by the tracking
       function */
    /* Use calloc to ensure uninitialized values are NULL */
    mxFree(field_numbers_ptr);
    ...
    track_table =
      (MYTRACK*)mxRealloc(track_table, num_elements*sizeof(MYTRACK));
    ...
    for (nelem=0; nelem<num_elements; nelem++) {
      ...
      if (!mxPassMethod)
	mexErrMsgIdAndTxt
	  ("Atpass:MissingPassMethod","Element # %d: Required field "
	   "'PassMethod' was not found in the element data structure", nelem);
      if (!mxIsChar(mxPassMethod))
	mexErrMsgIdAndTxt
	  ("Atpass:WrongPassMethod","Element # %d: 'PassMethod' field must be"
	   " a string", nelem);            
  ...
  if (nrhs >= 5) {    /* subtract 1 for C numbering: 0 to num_elements-1 */
    ...
    for (nref=0; nref<num_refpts; nref++)
      refpts[nref] = ((int)dblrefpts[nref])-1;
    if (num_refpts == 0)
  ...
  /* lost particle flag, initialized to 0 */
  mxLost=mxCreateLogicalMatrix(1,num_particles);
  /* turn number when lost */
  mxNturn=mxCreateDoubleMatrix(1,num_particles,mxREAL);
  /* element number when lost */
  mxNelem=mxCreateDoubleMatrix(1,num_particles,mxREAL);
  CoordDims[1] = num_particles;
  CoordDims[2] = lhist;
  LostCoordDims[1] = num_particles;
  /* Coordinates when particle is lost */
  mxLostCoord=mxCreateNumericArray(2,LostCoordDims,mxDOUBLE_CLASS,mxREAL);
  /* Coordinates before particle is lost */
  mxCoord=mxCreateNumericArray(3,CoordDims,mxDOUBLE_CLASS,mxREAL);
  ...
  for (turn=0; turn<num_turns; turn++) {
	...
	*elemdata =
	  (*track_elem)(*element, *elemdata, DblBuffer, num_particles,
			&paramStruct);
      }
      else if (*pass_elem) {               /* Pointer to a passFunction */
	*field_numbers =
	  (*pass_elem)(*element, *field_numbers, DblBuffer, num_particles,
		       pass_mode);
      }
      else {                               /* M-File */
	...
	DblBuffer=passhook(mxPassArg1, *element, mxPost);
      ...
      checkiflost(DblBuffer, num_particles, *xelmn, *xturn, xnturn, xnelem,
		  xcoord, xlostcoord, xlost, histbuf, ihist, lhist);
    ...
    if (pass_mode == MAKE_LOCAL_COPY) {             /* First turn */
      for (nelem=0; nelem<num_elements; nelem++) {
	if (field_numbers_ptr[nelem])
	  mexMakeMemoryPersistent(field_numbers_ptr[nelem]);
